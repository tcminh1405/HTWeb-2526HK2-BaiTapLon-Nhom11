...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (searchOverlay && searchInput) {
        // --- DATA SOURCE ---
        const hotels = [
            { name: "Vinpearl Luxury Nha Trang", type: "Khách sạn", url: "./booking.html" },
            { name: "Melia Vinpearl Empire Nha Trang", type: "Khách sạn", url: "./search-results.html" },
            { name: "Vinpearl Beachfront Nha Trang", type: "Khách sạn", url: "./search-results.html" },
            { name: "Hòn Tằm Resort", type: "Khách sạn", url: "./search-results.html" },
            { name: "Vinpearl Resort Nha Trang", type: "Khách sạn", url: "./search-results.html" }
        ];

        const rooms = [
            { name: "Biệt thự hướng vườn (Garden View Villa)", type: "Hạng phòng", hotel: "Vinpearl Luxury Nha Trang" },
            { name: "Biệt thự 2 tầng lớn (Grand 2-Story Villa)", type: "Hạng phòng", hotel: "Vinpearl Luxury Nha Trang" },
            { name: "Beach Front Villa", type: "Hạng phòng", hotel: "Vinpearl Luxury Nha Trang" },
            { name: "Presidential Suite", type: "Hạng phòng", hotel: "Vinpearl Luxury Nha Trang" }
        ];
        ...
        function saveHistory(query) {
            let history = JSON.parse(localStorage.getItem('vp_search_history') || '[]');
        ...
        function renderHistoryAndSuggestions() {
            ...
            if (query === '') {
                const history = JSON.parse(localStorage.getItem('vp_search_history') || '[]');
                if (history.length > 0) {
                    const historySection = document.createElement('div');
                    historySection.className = 'search-section';
                    historySection.innerHTML = '<div class="search-section-title">Lịch sử tìm kiếm</div>';
                    ...
                    history.forEach(item => {
                        ...
                        itemEl.querySelector('.remove-history').onclick = (e) => removeHistory(e, item);
                ...
                popularSection.innerHTML = '<div class="search-section-title">Gợi ý phổ biến</div>';
                const popularItems = ["Vinpearl Luxury Nha Trang", "Presidential Suite", "Spa", "Nhà hàng"];
                ...
                if (filtered.length > 0) {
                    ...
                    suggestionSection.innerHTML = '<div class="search-section-title">Kết quả gợi ý</div>';
                    ...
                    searchDropdown.innerHTML = '<div class="text-center py-4 text-muted">Không tìm thấy kết quả phù hợp</div>';
                }
            }
        }
    }
